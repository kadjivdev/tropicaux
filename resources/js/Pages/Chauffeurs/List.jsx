...
export default function List({ chauffeurs }) {
    ...
    const deleteUser = (e, chauffeur) => {
        e.preventDefault();
        Swal.fire({
            title: '<span style="color: #facc15;">⚠️ Êtes-vous sûr ?</span>', // yellow text
            text: `Le Chauffeur (${chauffeur.raison_sociale}) sera supprimé de façon permanente !`,
            ...
            if (result.isConfirmed) {
                ...
                destroy(route('chauffeur.destroy', chauffeur.id), {
                    onSuccess: () => {
                        Swal.close();
                        Swal.fire({
                            title: '<span style="color: #2a7348;">👌Suppression réussie </span>',
                            text: `Le Chauffeur (${chauffeur.raison_sociale}) a été supprimé avec succès.`,
        ...
        <AuthenticatedLayout
            header={
                <h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
                    <CIcon className='text-success' icon={cilList} /> Gestion des chauffeurs
                </h2>
            }
        >
            <Head title="Les Chauffeurs" />
                    ...
                    <div className="mx-auto _max-w-7xl space-y-6 sm:px-6 lg:px-8 " style={{ overflowX: 'auto' }} >
                        {checkPermission('chauffeur.create') ?
                            (<div className="text-center  items-center gap-4">
                                <Link className="btn w-50 bg-success bg-hover text-white" href={route("chauffeur.create")}> <CIcon className='' icon={cibAddthis} /> Ajouter</Link>
                            </div>) : null
                        }
                            ...
                            <tbody>
                                {
                                    chauffeurs.map((chauffeur, index) => (
                                        <tr key={chauffeur.id}>
                                            ...
                                            <td>{chauffeur.raison_sociale ?? '---'}</td>
                                            ...
                                            <td>
                                                <div className="dropstart">
                                                    ...
                                                    <ul className="dropdown-menu p-2 border rounded shadow">
                                                        <li>
                                                            {checkPermission('chauffeur.edit') ?
                                                                (<Link
                                                                    className='btn'
                                                                    href={route('chauffeur.edit', chauffeur.id)}
                                                                >
                                                        ...
                                                        <li>
                                                            {checkPermission('chauffeur.delete') ?
                                                                (<Link
                                                                    href="#"
                                                                    className='btn'
                                                                    onClick={(e) => deleteUser(e, chauffeur)}
                                                                >
