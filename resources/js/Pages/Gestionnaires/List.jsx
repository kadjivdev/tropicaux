...
export default function List({ gestionnaires }) {
    ...
    const deleteUser = (e, gestionnaire) => {
        e.preventDefault();
        Swal.fire({
            title: '<span style="color: #facc15;">⚠️ Êtes-vous sûr ?</span>', // yellow text
            text: `Le gestionnaire (${gestionnaire.raison_sociale}) sera supprimé de façon permanente !`,
            ...
            if (result.isConfirmed) {
                ...
                destroy(route('gestionnaire.destroy', gestionnaire.id), {
                    onSuccess: () => {
                        Swal.close();
                        Swal.fire({
                            title: '<span style="color: #2a7348;">👌Suppression réussie </span>',
                            text: `Le gestionnaire (${gestionnaire.raison_sociale}) a été supprimé avec succès.`,
        ...
        <AuthenticatedLayout
            header={
                <h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
                    <CIcon className='text-success' icon={cilList} /> Gestion des gestionnaires de fonds
                </h2>
            }
        >
            <Head title="Les Gestionnaires de fonds" />

            <div className="row py-12 justify-content-center">

                <div className="col-md-10 bg-white p-4 shadow sm:rounded-lg sm:p-8 dark:bg-gray-800">
                    <div className="mx-auto _max-w-7xl space-y-6 sm:px-6 lg:px-8 " style={{ overflowX: 'auto' }} >
                        {checkPermission('gestionnaire.create') ?
                            (<div className="text-center  items-center gap-4">
                                <Link className="btn w-50 bg-success bg-hover text-white" href={route("gestionnaire.create")}> <CIcon className='' icon={cibAddthis} /> Ajouter</Link>
                            </div>) : null
                        }
                                            ...
                                            <td>
                                                <div className="dropstart">
                                                    ...
                                                    <ul className="dropdown-menu p-2 border rounded shadow">
                                                        <li>
                                                            {checkPermission('gestionnaire.edit') ?
                                                                (<Link
                                                                    className='btn'
                                                                    href={route('gestionnaire.edit', gestionnaire.id)}
                                                                >
                                                        ...
                                                        <li>
                                                            {checkPermission('gestionnaire.delete') ?
                                                                (<Link
                                                                    href="#"
                                                                    className='btn'
                                                                    onClick={(e) => deleteUser(e, gestionnaire)}
                                                                >
