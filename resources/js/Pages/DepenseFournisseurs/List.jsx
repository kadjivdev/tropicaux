    ...
    const deleteDepense = (e, depense) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                destroy(route('depense-fournisseur.destroy', { "depense_fournisseur": depense.id }), {
                    onSuccess: () => {
                        Swal.close();
                        Swal.fire({
    ...
    const validatedDepense = (e, depense) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                patch(route('depense-fournisseur.validate', depense.id), {
                    onSuccess: () => {
                        Swal.close();
                        Swal.fire({
        ...
        <AuthenticatedLayout
            header={
                <h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
                    <CIcon className='text-success' icon={cilList} /> Gestion des dépenses fournisseurs
                </h2>
            }
        >
            <Head title="Les depenses fournisseur" />
                    ...
                    <div className="mx-auto _max-w-7xl space-y-6 sm:px-6 lg:px-8 " style={{ overflowX: 'auto' }} >
                        {checkPermission('depense.superviseur.create') ?
                            (<div className="text-center  items-center gap-4">
                                <Link className="btn w-50 bg-success bg-hover text-white" href={route("depense-fournisseur.create")}> <CIcon className='' icon={cibAddthis} /> Ajouter</Link>
                            </div>) : null
                        }
                            ...
                            <tbody>
                                {
                                    depenses.data.map((depense, index) => (
                                        <tr key={depense.id}>
                                                                ...
                                                                <li>
                                                                    {checkPermission('depense.superviseur.edit') ?
                                                                        (<Link
                                                                            ...
                                                                            href={route('depense-fournisseur.edit', depense.id)}
                                                                        >
                                            ...
                                            <td><span className="badge bg-light rounded text-dark rounded shadow-sm"> {depense.fournisseur?.raison_sociale ?? '---'}</span> </td>
                                            ...
                                            <td> <span className="badge bg-light border rounded text-dark">{`${depense.validatedAt || '--'}`}</span> </td>
