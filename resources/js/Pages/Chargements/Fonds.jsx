                ...
                <div className="col-md-10 bg-white p-4 shadow sm:rounded-lg sm:p-8 dark:bg-gray-800">
                    <div className="mx-auto _max-w-7xl space-y-6 sm:px-6 lg:px-8 " style={{ overflowX: 'auto' }} >
                        ...
                        <table className="table table-striped" id='myTable' style={{ width: '100%' }}>
                            ...
                            <tbody>
                                {
                                    chargement.fonds?.length > 0 &&
                                    chargement.fonds?.map((fond, index) => (
                                        <tr key={fond.id}>
                                            ...
                                            <td> <span className="badge bg-light border rounded text-dark">{`${fond.created_by?.firstname} - ${fond.created_by?.lastname}`}</span> </td>
                                            <td> <span className="badge bg-light border rounded text-dark">{`${fond.validated_at || '--'}`}</span> </td>
                                            <td> <span className="badge bg-light border rounded text-dark">{`${fond.validated_by?.firstname || ''} - ${fond.validated_by?.lastname || ''}`}</span> </td>
                                        </tr>
                                    ))
                                }
                            </tbody>
