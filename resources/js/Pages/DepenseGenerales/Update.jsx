...
export default function Update({ depense, types }) {
    ...
    const submit = (e) => {
        ...
        patch(route('depense-generale.update', { "depense_generale": depense.id }), {
            onStart: () => {
                Swal.fire({
        ...
        <AuthenticatedLayout
            header={
                <h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
                    <CIcon className='text-success' icon={cilPencil} /> Modifier la dépense <span className="badge bg-light border rounded text-success">{depense.reference}</span>
                </h2>
            }
        >
                        ...
                        <div className="bg-light p-3 rounded border mb-5">
                            {checkPermission('depense.superviseur.view') ?
                                (<div className=" text-center  items-center gap-4">
                                    <Link className="btn btn-sm bg-success bg-hover text-white" href={route("depense-generale.index")}> <CIcon icon={cilList} /> Liste des dépenses generales</Link>
                                </div>) : null
                            }
