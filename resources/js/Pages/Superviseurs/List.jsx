...
export default function List({ superviseurs }) {
    ...
    const deleteUser = (e, superviseur) => {
        e.preventDefault();
        Swal.fire({
            title: '<span style="color: #facc15;">⚠️ Êtes-vous sûr ?</span>', // yellow text
            text: `Le superviseur (${superviseur.raison_sociale}) sera supprimé de façon permanente !`,
            ...
            if (result.isConfirmed) {
                ...
                destroy(route('superviseur.destroy', superviseur.id), {
                    onSuccess: () => {
                        Swal.close();
                        Swal.fire({
                            title: '<span style="color: #2a7348;">👌Suppression réussie </span>',
                            text: `Le superviseur (${superviseur.raison_sociale}) a été supprimé avec succès.`,
        ...
        <AuthenticatedLayout
            header={
                <h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
                    <CIcon className='text-success' icon={cilList} /> Gestion des superviseurs
                </h2>
            }
        >
            <Head title="Les Superviseurs" />
                    ...
                    <div className="mx-auto _max-w-7xl space-y-6 sm:px-6 lg:px-8 " style={{ overflowX: 'auto' }} >
                        {checkPermission('superviseur.create') ?
                            (<div className="text-center  items-center gap-4">
                                <Link className="btn w-50 bg-success bg-hover text-white" href={route("superviseur.create")}> <CIcon className='' icon={cibAddthis} /> Ajouter</Link>
                            </div>) : null
                        }
                        <table className="table table-striped" id='myTable' style={{ width: '100%' }}>
                            ...
                            <tbody>
                                {
                                    superviseurs.map((superviseur, index) => (
                                        <tr key={superviseur.id}>
                                            <th scope="row">{index + 1}</th>
                                            <td>{superviseur.raison_sociale ?? '---'}</td>
                                            <td>{superviseur.phone || '---'}</td>
                                            <td>{superviseur.email || '---'}</td>
                                            <td>{superviseur.adresse || '---'}</td>
                                            <td>
                                                <div className="dropstart">
                                                    ...
                                                    <ul className="dropdown-menu p-2 border rounded shadow">
                                                        <li>
                                                            {checkPermission('superviseur.edit') ?
                                                                (<Link
                                                                    className='btn'
                                                                    href={route('superviseur.edit', superviseur.id)}
                                                                >
                                                        ...
                                                        <li>
                                                            {checkPermission('superviseur.delete') ?
                                                                (<Link
                                                                    href="#"
                                                                    className='btn'
                                                                    onClick={(e) => deleteUser(e, superviseur)}
                                                                >
